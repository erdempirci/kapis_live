...
type GameState={matchupId:string;title:string;choice:'left'|'right';choiceLabel:string;otherLabel:string;leftVotes:number;rightVotes:number;totalVotes:number;difference:number;lastHourVotes:number;myInvites:number;mySameSide:number;challengeToken:string}|null;
...
export default function SwipeEnhancer(){
 ...
 function patchFeedbackCopy(){document.querySelectorAll<HTMLElement>('.feedbackSheet').forEach(sheet=>{const h2=sheet.querySelector('h2'),p=sheet.querySelector('p'),label=sheet.querySelector('label');if(!h2)return;const text=h2.textContent||'';if(text.includes('İlk 5 Kapışını')){h2.textContent='İlk 10 oyunu tamamladın 🎉';if(p)p.textContent='Kapış’ı biraz kullandın. İlk izlenimini yalnızca bir kez soruyoruz.';if(label)label.textContent='Şu an ilk değiştirmemizi istediğin şey ne?'}else if(text.includes('50 Kapış yaptın')){h2.textContent='Kapış’ta 10. kullanım günün 🔥';if(p)p.textContent='İlk görüşüne hâlâ katılıyor musun? Bu, sana soracağımız ikinci ve son genel ürün geri bildirimi.';if(label)label.textContent='10 gün sonra neyi değiştirmemizi istersin?'}})}
 ...
 function bindTopCard(){
  ...
  const onUp=(e:PointerEvent)=>{if(!start.current||interactionLocked.current)return;const dx=e.clientX-start.current.x,dy=e.clientY-start.current.y;start.current=null;const w=Math.max(card.clientWidth,280),horizontal=Math.abs(dx)>=Math.abs(dy),moved=Math.hypot(dx,dy)>10;if(!horizontal&&dy<-95){interactionLocked.current=true;suppressClickUntil.current=Date.now()+700;track('swipe_up');animateOut(card,0,-window.innerHeight*.45);setTimeout(()=>{card.remove();interactionLocked.current=false},220);return resetClasses(card)}if(horizontal&&Math.abs(dx)/w>=.32){interactionLocked.current=true;suppressClickUntil.current=Date.now()+800;const buttons=[...card.querySelectorAll<HTMLButtonElement>('.duel button')].sort((a,b)=>a.getBoundingClientRect().left-b.getBoundingClientRect().left);const chosen=dx<0?buttons[0]:buttons[buttons.length-1];track(dx<0?'swipe_left':'swipe_right');animateOut(card,dx<0?-window.innerWidth*1.2:window.innerWidth*1.2,dy);setTimeout(()=>{if(chosen&&!chosen.disabled){programmaticVote.current=true;chosen.click();programmaticVote.current=false;showUndo();void openGame(card,chosen)}},100);return}if(moved)suppressClickUntil.current=Date.now()+350;card.style.transform='';resetClasses(card)};
 ...
 async function openGame(card:HTMLElement,btn:HTMLButtonElement){
  ...
  const incoming=localStorage.getItem('kapis_challenge_token');if(incoming){await supabase.rpc('kapis_record_challenge_join',{p_token:incoming,p_choice:actualSide});localStorage.removeItem('kapis_challenge_token')}
  const [{data:s},{data:ch}]=await Promise.all([supabase.rpc('kapis_competition_snapshot',{p_matchup_id:m.id}),supabase.rpc('kapis_create_challenge',{p_matchup_id:m.id,p_choice:actualSide})]);
  const x=s?.[0],challengeToken=ch?.[0]?.token||'';if(!x)return;
  setShareStatus('');
  setGame({matchupId:m.id,title,choice:actualSide,choiceLabel,otherLabel:actualSide==='left'?m.right_label:m.left_label,leftVotes:Number(x.left_votes),rightVotes:Number(x.right_votes),totalVotes:Number(x.total_votes),difference:Number(x.difference),lastHourVotes:Number(x.last_hour_votes),myInvites:Number(x.my_invites),mySameSide:Number(x.my_same_side),challengeToken});track('results_viewed',{mode:'competition'});
 }
 ...
 function challenge(){
  if(!game)return;
  const token=game.challengeToken;
  if(!token){setShareStatus('Meydan okuma bağlantısı hazırlanamadı. Tekrar dene.');return}
  const url=`${location.origin}/c/${token}`;
  const mine=game.choice==='left'?game.leftVotes:game.rightVotes,other=game.choice==='left'?game.rightVotes:game.leftVotes;
  const scoreLine=mine===other?'Skor tam başa baş 😱':mine<other?`${game.otherLabel} sadece ${other-mine} oy önde 😱`:`${game.choiceLabel} ${mine-other} oy önde 🔥`;
  const text=`Ben ${game.choiceLabel} tarafındayım 🔥\n${scoreLine}\nSen hangi taraftasın? Gel kapışalım ⚔️`;
  if(navigator.share){
    navigator.share({title:'KAPIŞ — Meydan Okuma',text,url}).then(()=>{track('shared',{kind:'challenge'});setShareStatus('Meydan okuma gönderildi ⚔️')}).catch((e:any)=>{if(e?.name!=='AbortError')copyChallenge(text,url)});
  }else copyChallenge(text,url);
 }

 function copyChallenge(text:string,url:string){
  const full=`${text}\n${url}`;
  if(navigator.clipboard?.writeText){navigator.clipboard.writeText(full).then(()=>{track('shared',{kind:'challenge',fallback:'clipboard'});setShareStatus('Meydan okuma kopyalandı. WhatsApp’a yapıştırabilirsin ⚔️')}).catch(()=>legacyCopy(full));}
  else legacyCopy(full);
 }
 function legacyCopy(text:string){const ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.focus();ta.select();try{document.execCommand('copy');track('shared',{kind:'challenge',fallback:'legacy'});setShareStatus('Meydan okuma kopyalandı. WhatsApp’a yapıştırabilirsin ⚔️')}catch{setShareStatus('Paylaşım açılamadı. Tekrar dene.')}finally{ta.remove()}}
