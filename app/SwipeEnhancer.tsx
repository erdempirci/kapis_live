...
export default function SwipeEnhancer(){
 ...
 function bindTopCard(){
  ...
  const onDown=(e:PointerEvent)=>{
   ...
   if(target.closest('input,textarea,a,.shareMini'))return;
  ...
  const onMove=(e:PointerEvent)=>{
   if(!start.current||interactionLocked.current)return;
   const dx=e.clientX-start.current.x,dy=e.clientY-start.current.y;
   const w=Math.max(card.clientWidth,280);
   const horizontal=Math.abs(dx)>=Math.abs(dy);
   if(Math.abs(dx)>7||Math.abs(dy)>7)e.preventDefault();
   card.style.transform=`translate3d(${dx}px,${Math.min(dy,70)}px,0) rotate(${dx/28}deg)`;
   const ratio=Math.abs(dx)/w;
   card.classList.toggle('dragLeft',horizontal&&dx<0);
   card.classList.toggle('dragRight',horizontal&&dx>0);
   card.classList.toggle('dragUp',!horizontal&&dy<-45);
   if((ratio>.32||dy<-95)&&!thresholdBuzzed.current){
  ...
  const onUp=(e:PointerEvent)=>{
   ...
   if(!horizontal&&dy<-95){
    ...
    track('swipe_up');
    animateOut(card,0,-window.innerHeight*.45);
    setTimeout(()=>{card.remove();interactionLocked.current=false},220);
    return resetClasses(card);
   }

   if(horizontal&&Math.abs(dx)/w>=.32){
    interactionLocked.current=true;
    suppressClickUntil.current=Date.now()+800;
    const buttons=[...card.querySelectorAll<HTMLButtonElement>('.duel button')];
    const visuallySorted=buttons.sort((a,b)=>a.getBoundingClientRect().left-b.getBoundingClientRect().left);
    const chosen=dx<0?visuallySorted[0]:visuallySorted[visuallySorted.length-1];
    track(dx<0?'swipe_left':'swipe_right');
    animateOut(card,dx<0?-window.innerWidth*1.2:window.innerWidth*1.2,dy);
    setTimeout(()=>{
      if(chosen&&!chosen.disabled){
       programmaticVote.current=true;
       chosen.click();
       programmaticVote.current=false;
       showUndo();
       showResult(card);
      }
    },100);
  ...
  const onClick=(e:MouseEvent)=>{
   const btn=(e.target as HTMLElement).closest<HTMLButtonElement>('.duel button');
   if(!btn)return;
   if(programmaticVote.current)return;
   if(Date.now()<suppressClickUntil.current||interactionLocked.current){
    e.preventDefault();
    e.stopPropagation();
   ...
   showResult(card);
   interactionLocked.current=true;
   setTimeout(()=>{interactionLocked.current=false},350);
  };
 ...
 function prepareStableOrder(card:HTMLElement){
  ...
  let swap=localStorage.getItem(key);
  if(swap===null){swap=Math.random()<.5?'1':'0';localStorage.setItem(key,swap)}
  const duel=card.querySelector<HTMLElement>('.duel');if(!duel)return;
  const children=[...duel.children] as HTMLElement[];if(children.length<3)return;
