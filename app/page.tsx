...
const emoji:Record<string,string> = { Futbol:'⚽', Şehir:'🏙️', Teknoloji:'📱', Üniversite:'🎓', Yemek:'🍔', Müzik:'🎵', Film:'🎬', Oyun:'🎮' };
...
export default function Page(){
  ...
  async function boot(){
    try{
      ...
      if(ref && ref!==currentUser.id) localStorage.setItem('kapis_referrer',ref);

      await supabase.from('kapis_profiles').upsert({user_id:currentUser.id,nickname:`Kapışçı #${currentUser.id.replaceAll('-','').slice(-5)}`},{onConflict:'user_id',ignoreDuplicates:true});
      await Promise.all([loadProfile(currentUser.id),loadMatchups(currentUser.id),loadLeaderboard(),loadReferralCount(currentUser.id)]);
    }catch(e:any){setAuthError(e?.message||'Bağlantı kurulamadı.');}
  ...
  async function loadReferralCount(uid:string){
    const {count}=await supabase.from('kapis_referrals').select('id',{count:'exact',head:true}).eq('inviter_id',uid);
  ...
  async function loadMatchups(uid:string){
    ...
    setMatchups(ms??[]);
    const nextCounts:Counts={};for(const c of cs??[]) nextCounts[c.matchup_id]={left_votes:Number(c.left_votes),right_votes:Number(c.right_votes)};setCounts(nextCounts);
    const nextChoices:ChoiceMap={};for(const v of vs??[]) nextChoices[v.matchup_id]=v.choice;setChoices(nextChoices);
  }
  ...
  async function claimPendingReferral(){
    const inviter=localStorage.getItem('kapis_referrer');
    if(!inviter||!user||inviter===user.id) return false;
    const {data,error}=await supabase.rpc('kapis_claim_referral',{p_inviter_id:inviter});
    if(!error){localStorage.removeItem('kapis_referrer');return !!data?.length;}
    return false;
  }
  ...
  async function vote(m:Matchup,side:'left'|'right'){
    ...
    if(profile.credits<=0){flash('Kredin bitti. Arkadaş davet ederek kredi kazanabilirsin.');return;}
    ...
    const referralClaimed=await claimPendingReferral();
    if(nextTotal>=5&&!feedbackDone) setTimeout(()=>setFeedbackOpen(true),500);
    void loadLeaderboard();
    flash(referralClaimed?'Tahmin kaydedildi ✅ Davetin tamamlandı; arkadaşın +10 kredi kazandı.':'Tahminin canlı sisteme kaydedildi ✅');
  }

  async function shareInvite(){
    if(!user) return;
    const url=`${window.location.origin}/?ref=${user.id}`;
    const text='KAPIŞ’a gel, tarafını seç ve tahminini yap 🔥 İlk kullanıcıya 100 kredi var.';
    try{
      if(navigator.share){await navigator.share({title:'KAPIŞ',text,url});flash('Davet bağlantısı hazır 🚀');}
      else{await navigator.clipboard.writeText(url);flash('Davet linki kopyalandı ✅');}
    }catch(e:any){if(e?.name!=='AbortError') flash('Paylaşım açılamadı.');}
  }
  ...
  function flash(t:string){setToast(t);setTimeout(()=>setToast(''),2800);}
  ...
  if(authError) return <main className="center"><div className="brand">KAPIŞ</div><h2>Bağlantı ayarı gerekiyor</h2><p className="muted">{authError}</p></main>;

  return <main>
    <header><div><div className="brand">KAPIŞ</div><div className="tag">Tarafını seç • sonucu tahmin et</div></div><div className="credit">🎁 {profile?.credits??100} kredi</div></header>
    ...
    {tab==='home'&&<>
      ...
      <div className="invite"><div><span>🎁 ARKADAŞ DAVETİ</span><h3>Gönder, +10 kredi kazan</h3><p>Arkadaşın linkinden gelip ilk Kapışını yaptığında hesabına otomatik 10 kredi eklenir.</p></div><button onClick={shareInvite}>📤 Arkadaşıma gönder</button><small>{referrals} arkadaş katıldı • +{referrals*10} kredi kazandın</small></div>
      <div className="live"><i/> CANLI BETA • {totalVotes.toLocaleString('tr-TR')} oy toplandı</div>
      <h2>🔥 Şu an Kapışanlar</h2>
      <div className="cards">{matchups.map(m=>{const c=counts[m.id]??{left_votes:0,right_votes:0};const total=c.left_votes+c.right_votes;const lp=total?Math.round(c.left_votes/total*100):50;const choice=choices[m.id];const end=new Date(m.ends_at);const hours=Math.max(0,Math.ceil((end.getTime()-Date.now())/3600000));return <article key={m.id}><div className="meta"><span>{emoji[m.category]??'🔥'} {m.category}</span><small>⏱ {hours} saat</small></div><div className="duel"><button className={choice==='left'?'picked':''} onClick={()=>vote(m,'left')}><strong>{m.left_label}</strong><em>%{lp}</em><small>{c.left_votes.toLocaleString('tr-TR')} güç</small></button><b>VS</b><button className={choice==='right'?'picked':''} onClick={()=>vote(m,'right')}><strong>{m.right_label}</strong><em>%{100-lp}</em><small>{c.right_votes.toLocaleString('tr-TR')} güç</small></button></div><div className="bar"><i style={{width:`${lp}%`}}/></div><div className="status">{choice?`Tahminin: ${choice==='left'?m.left_label:m.right_label}`:'Tarafını seç ve sonucu tahmin et'}</div></article>})}</div>
    </>}
