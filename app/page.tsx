...
type Matchup={id:string;title:string;category:string;left_label:string;right_label:string;ends_at:string;matchup_type:'tercih'|'gorus'|'tahmin';pos?:number};
...
export default function Page(){
 ...
 const [stats,setStats]=useState<Stats|null>(null),[expertise,setExpertise]=useState<Expertise[]>([]),[referrals,setReferrals]=useState(0),[tab,setTab]=useState<'home'|'rank'|'profile'|'settings'>('home'),[feed,setFeed]=useState<'voted'|'mine'|'all'>('mine');
 ...
 async function boot(){try{
  ...
  await Promise.all([loadInterests(u.id),loadMatchups(u.id),loadLeaderboard(),loadReferralCount(u.id),loadStats(),loadExpertise(),loadPackage(),checkFeedback()]);
 ...
 async function loadMatchups(uid:string){const [{data:ms,error:me},{data:cs,error:ce},{data:vs,error:ve}]=await Promise.all([
  supabase.from('kapis_matchups').select('id,title,category,left_label,right_label,ends_at,matchup_type').eq('status','active').lte('starts_at',new Date().toISOString()).gt('ends_at',new Date().toISOString()),
 ...
 async function vote(m:Matchup,side:'left'|'right'){
  ...
  await Promise.all([loadStats(),loadLeaderboard(),loadExpertise(),checkFeedback()]);
  if(pkg?.[0]?.completed&&Number(pkg[0].reward_credits)>0){setProfile(p=>p?{...p,credits:p.credits+Number(pkg[0].reward_credits)}:p);flash(`Paket tamamlandı! +${pkg[0].reward_credits} kredi 🎁`)}
  else if(ref?.[0]?.completed){setProfile(p=>p?{...p,credits:Number(ref[0].invitee_credits)}:p);localStorage.removeItem('kapis_referrer');flash('5 oyun tamamlandı! Davet bonusu +10 kredi 🎁')}
  ...
  else flash(`Oyun kaydedildi ✅ +${Number(row?.awarded_xp??1)} XP`);
 }

 async function shareMatchup(m:Matchup){const url=`${location.origin}/?matchup=${m.id}`;const text=`KAPIŞ: ${m.title} — Sen hangi taraftasın?`;try{if(navigator.share)await navigator.share({title:'KAPIŞ',text,url});else await navigator.clipboard.writeText(url);if(navigator.onLine)void supabase.rpc('kapis_record_share',{p_matchup_id:m.id});flash('Kapış paylaşım bağlantısı hazır 🚀')}catch(e:any){if(e?.name!=='AbortError')flash('Paylaşım açılamadı.')}}
